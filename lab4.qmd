...
format:
   html:
    ...
    self-contained: true
    toc: true
subtitle: 'Introduction to Statistics in R | Brandon Pizer'
---
...
```{r}
sally_table <- table(sally_clean$section, sally_clean$unittype) 
  
chisq.test(sally_table)



```
**Answer** 
the p-value (2.2e-16) is less than 0.05 which indicates that their is a significant relationship between forest type and channel unit.
...
salamander_cc <- and_vertebrates |>
  filter(species == "Coastal giant salamander" & section %in% c("CC"))

salamander_OG <- and_vertebrates |>
  filter(species == "Coastal giant salamander" & section %in% c("OG"))
  

shapiro.test(salamander_cc$weight_g)
shapiro.test(salamander_OG$weight_g)

# data is not normal distribution 

wilcox.test(weight_g ~ section, data = sally_clean)


```
**Answer**
The p-value for the wilcox test was (9.57e-07) which is less than 0.05. This indicates that there is a significant difference in species biomass between clear cut and old growth forest types
...
cutthroat_sample <- and_vertebrates %>%
  filter(species == "Cutthroat trout") %>%
  slice_sample(n = 5000)


shapiro.test(cutthroat_sample$length_1_mm)
shapiro.test(cutthroat_sample$weight_g)

cor.test(cutthroat_sample$length_1_mm, cutthroat_sample$weight_g, method = "spearman")

ggplot(cutthroat_sample, aes(x = length_1_mm, y = weight_g)) +
  geom_point(alpha = 0.5, color = "blue") +  # Scatter points with transparency
  geom_smooth(method = "lm", color = "red", se = TRUE) +  # Linear regression trend line
  labs(title = "Correlation Between Body Length and Mass in Cutthroat Trout",
       x = "Body Length (mm)",
       y = "Body Mass (g)") +
  theme_minimal()
...
```
**Answer**
Since the p-value (2.2e-16) is less than 0.05, the data indicates that there is a significant correlation between body mass and body length
...
anova_model <- aov(size ~ site, data = pie_crab)
...
ggplot(pie_crab, aes(x = factor(site, levels = site_order), y = size)) +
...
```
**Answer** 
A one-way ANOVA was conducted to compare differences in carapace width across three different sites. The results indicated a significant effect of site on carapace width (F(df, df) = 9.22, p = 1.15e-15), suggesting that crab size differs among locations.
...
lm(size ~ water_temp_sd, data = pie_crab) |>
  summary()


```
**Answer**
A simple linear regression was conducted to examine the effect of annual variation in water temperature (water_temp_sd) on carapace width. The model was not significant (F(1, 390) = 0.3999, p = 0.5275), explaining R² = 0.001024 of the variance in carapace width. The regression coefficient for water_temp_sd was B = 0.09938 (p = 0.528), indicating that for each unit increase in temperature variation, carapace width increased/decreased by about 0.1 mm.
...
ggplot(pie_crab, aes(x = water_temp_sd, y = size)) +
  ...
  labs(title = "Effect of Water Temperature Variation on Carapace Width",
...
```
**Answer**
The correlation matrix revealed that latitude and air_temp_sd were correlated (r = 0.79), while latitude and water_temp_sd showed almost no correlation (r = 0.04). The relationship between air_temp_sd and water_temp_sd was weakly correlated (r = 0.41), suggesting low multicollinearity concerns.

```{r}

lm(size ~ latitude + air_temp_sd + water_temp_sd, data = pie_crab) |>
  summary()


```
**Answer**
A multiple linear regression was conducted to examine the effects of latitude, air temperature variation (air_temp_sd), and water temperature variation (water_temp_sd) on crab size. The overall model was significant (F(3, 388) = 70.13, p = 2.2e-16), explaining R² = 0.35 of the variance in size.
